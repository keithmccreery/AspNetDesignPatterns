...
- `tests/AspNetDesignPatterns.Api.Shared.Tests/OpenApi/BearerSecuritySchemeTransformerTests.cs` — the transformer adds the
  bearer scheme to `components` and a document-wide security requirement referencing it.
- `tests/AspNetDesignPatterns.Api.Shared.Tests/OpenApi/OpenApiExposureTests.cs` — `MapApiReference` serves the document
