...
One folder per concern (mirroring `Shared/Pipeline/`'s shape — one cohesive folder per
concern, not a flat bucket): each pairs the contract with the registration extension that
scans for it, and — where the contract has a shape of its own to explain — its `SettingsBase<T>`
...
| `RequestHandlers/` | (contract is `IRequestHandler<,>`, in `Shared/Handlers/`) | `RequestHandlerRegistration`: `AddRequestHandlers()` |
...
assemblies" step all five registration extensions above (and `Shared/Pipeline`'s
...
`SettingsBase<T>`, …) stay in `AspNetDesignPatterns.Api.DependencyInjection` regardless of
...
| `AddPipeline()` / `AddPipelineSteps(assemblies)` | (see [`../Shared/Pipeline`](../Shared/Pipeline/README.md)) | factory + steps (scoped) |
...
1. Create `Features/<Name>/` (in `AspNetDesignPatterns.Api`, not here).
...
`tests/AspNetDesignPatterns.Api.Shared.Tests/DependencyInjection/` mirrors this folder's
...
Shared-only test can't prove — is `ReflectionRegistrationTests` in
